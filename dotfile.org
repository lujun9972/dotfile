...
#+BEGIN_SRC lua :tangle "~/.config/awesome/rc.lua"
  awful.screen.connect_for_each_screen(function(s)
      ...
      awful.tag({ "1.HOME", "2.STUDIO", "3.GAMES", "4.EDIT", "5.VM", "6.IRC", "7", "8", "9" }, s, awful.layout.layouts[1])
...
client鼠标操作是当有应用获取到焦点时触发的操作，awesome将当前捕获到焦点的应用作为参数传递给对应的函数。
#+BEGIN_SRC  lua :tangle "~/.config/awesome/rc.lua"
...
client鼠标操作需要通过应用的buttons属性来设置，参见后面的Rules章节
*** 设置快捷键
每个快捷键都保存为一个key对象。
...
#+END_SRC
**** 设置client快捷键
client快捷键是当有焦点在应用程序(client)上时才能触发的。这时awesome调用快捷键上的函数时会将当前应用程序对象作为参数传递过去。
#+BEGIN_SRC  lua :tangle "~/.config/awesome/rc.lua"
...
client快捷键需要通过对应用的keys属性来进行设置(参见Rules章节)
*** Rules
=awful.rules.rules= 是全局的规则表，这里定义的规则适用于所有新开的应用。
...
+ =properties= 定义了应用的属性,若属性值为函数，则该函数的返回值会作为属性的值。
+ =callback= 定义了触发的回调函数,该回调函数会接受打开的应用作为参数
...
#+END_SRC
*** Signal
注册事件发生时的触发函数,该函数接受一个应用(client对象)作为参数
#+BEGIN_SRC lua :tangle "~/.config/awesome/rc.lua"
  -- {{{ Signals
